...
resource "aws_api_gateway_resource" "restaurante34-api_delete_resource" {
  rest_api_id = aws_api_gateway_rest_api.restaurante34-api.id
  parent_id   = aws_api_gateway_rest_api.restaurante34-api.root_resource_id
  path_part   = "delete"  
}

resource "aws_api_gateway_method" "restaurante34-api_delete_method" {
  rest_api_id   = aws_api_gateway_rest_api.restaurante34-api.id
  resource_id   = aws_api_gateway_resource.restaurante34-api_delete_resource.id
  http_method   = "POST"  
  authorization = "NONE" 
}
resource "aws_api_gateway_integration" "restaurante34-api_delete" {
  rest_api_id = aws_api_gateway_rest_api.restaurante34-api.id
  resource_id = aws_api_gateway_resource.restaurante34-api_delete_resource.id
  http_method = aws_api_gateway_method.restaurante34-api_delete_method.http_method
  type        = "AWS_PROXY"
  integration_http_method = "POST"
  uri         = aws_lambda_function.terraform_delete.invoke_arn
}

resource "aws_lambda_permission" "terraform_lambda_permission_delete" {
  statement_id  = "AllowAPIGatewayInvoke"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.terraform_delete.function_name
  principal     = "apigateway.amazonaws.com"
...
data "archive_file" "lambda_terraform_delete" {
  type = "zip"
...
resource "aws_lambda_function" "terraform_delete" {
  function_name = "delete"
  filename      = "delete.zip"
  runtime = "nodejs20.x"
  handler = "index.handler"
  source_code_hash = data.archive_file.lambda_terraform_delete.output_base64sha256
  role             = aws_iam_role.lambda_exec.arn
